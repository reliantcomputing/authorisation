...
class PatternController @Inject()(deadbolt: DeadboltActions, actionBuilders: ActionBuilders) extends Controller {
  ...
  def printersDelele = deadbolt.Pattern("printers.delete", PatternType.EQUALITY)() { authRequest =>
    Future {
             Ok(accessOk())
           }
                                                                             }
  ...
  def printersFoo_fromBuilder = actionBuilders.PatternAction("printers.update", PatternType.EQUALITY).defaultHandler() { authRequest =>
    Future {
             Ok(accessOk())
           }
                                                                                                                    }
